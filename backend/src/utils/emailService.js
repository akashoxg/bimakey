...
www.bimakey.in | hello@bimakey.in
    ...
    if (lead.email && lead.email.trim()) {
      await transporter.sendMail({
        from: `"BimaKey Support" <${config.SMTP_USER}>`,
        to: lead.email.trim(),
        ...
        html: `
          <div style="font-family: Arial, sans-serif; padding: 25px; max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 12px; border: 1px solid #e5e7eb;">
            ...
            <div style="padding: 20px 10px; color: #374151; line-height: 1.6;">
              ...
              <p>Thanks for reaching out and connecting with BimaKey! We have successfully received your query regarding <strong>${getInsuranceLabel(lead.insuranceType || lead.claimType)}</strong>.</p>
...
/**
 * Send formatted claim notification email
 */
const sendClaimNotification = async (formData) => {
  try {
    if (!config.SMTP_HOST || !config.SMTP_USER || !config.SMTP_PASS) {
      console.log('SMTP credentials not configured. Skipping email notification.');
      return { sent: false, reason: 'not_configured' };
    ...
    // Build claim-specific fields HTML
    let claimFieldsHTML = '';
    let claimFieldsText = '';
    ...
    switch (claimType) {
      ...
      case 'motor-claim':
        const motorClaimLabels = {
          'own-damage': 'Own Damage (OD)',
          'third-party': 'Third Party Liability',
          'theft': 'Theft/Total Loss',
      ...
      case 'renewal-help':
        const renewalLabels = {
          ...
          'porting': 'Port to New Plan',
          'upgrade': 'Upgrade Coverage',
          'compare': 'Compare Other Plans',
          'other': 'Other',
        };
        claimFieldsHTML = `
    ...
    const mailOptions = {
            ...
            <div style="padding: 30px;">
              ...
              <div style="background: #f9fafb; border-radius: 12px; overflow: hidden; border: 1px solid #e5e7eb; margin-bottom: 20px;">
                <table style="width: 100%; border-collapse: collapse;">
                  ...
                  <tbody>
                    <tr>
                      <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; font-weight: bold; color: #374151; width: 35%;">Full Name</td>
                      <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; font-size: 15px; font-weight: 600; color: #111827;">${name}</td>
                    </tr>
                    <tr>
                      <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; font-weight: bold; color: #374151;">Phone</td>
                      <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;"><a href="tel:+91${phone}" style="color: ${claimInfo.color}; text-decoration: none; font-weight: 600;">📞 +91 ${phone}</a></td>
                    </tr>
                    <tr>
                      <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; font-weight: bold; color: #374151;">Email</td>
                      <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;"><a href="mailto:${email}" style="color: ${claimInfo.color}; text-decoration: none;">✉️ ${email}</a></td>
                    </tr>
                    ...
                    <tr>
                      <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; font-weight: bold; color: #374151;">Callback</td>
                      <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; color: #7c3aed; font-weight: 600;">${getCallbackTimeLabel(callbackTime)}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
      ...
      `,
      text: `
${claimInfo.emoji} NEW ${claimInfo.label.toUpperCase()} - BIMARKEY
${'═'.repeat(45)}
...
www.bimakey.in | hello@bimakey.in
    ...
    if (email && email.trim()) {
      await transporter.sendMail({
        from: `"BimaKey Claims Support" <${config.SMTP_USER}>`,
        to: email.trim(),
...
export { sendLeadNotification, sendClaimNotification };
